...
SDL_Point get_spawn_point(TileMap *tilemap, int ishunter) {
    ...
    if (ishunter) {
        if (tilemap->hunter_spawn.num_points > 0) {
            int index = rand() % tilemap->hunter_spawn.num_points;
            return tilemap->hunter_spawn.points[index];
        }
    } else {
        if (tilemap->human_spawn.num_points > 0) {
            int index = rand() % tilemap->human_spawn.num_points;
            // int index = 0;
            return tilemap->human_spawn.points[index];
        }
    }
        ...
        for (int x = 0; x < width; x++)
        {
            ...
            if (type != 1)
            {
                ...
                if (type == 2)
                {
                    ...
                    // tilemap->human_spawn.points[human_spawn_count++] = (SDL_Point){(x + 1) * MAZE_SCALEUP_FACTOR*tilemap->tile_size, y * MAZE_SCALEUP_FACTOR*tilemap->tile_size};
                }
                else if (type == 3)
                {
                ...
                for (int i = 0; i < MAZE_SCALEUP_FACTOR; i++)
                {
                    for (int j = 0; j < MAZE_SCALEUP_FACTOR; j++)
                    {
                        int new_x = (x+1) * MAZE_SCALEUP_FACTOR + i;
                        int new_y = y * MAZE_SCALEUP_FACTOR + j;
